...
spand = 50;
ind = 1;
for j = to(1):to(1)+100 %to(4)+10:to(4)+10%1:spand:17000 % 1:100:70000%:1200%(max(size(t_sol))-tl)
i = j+0;
     clf
% subplot(2, 2, [1 3])
...
%       hold on 
     if (j+tl >= to(1))
         marker = 2;
       for nTraj = 1:16
             ft = plot3(store_Traj{marker,4}(j:j+tl,plotdofs(1)+(nTraj-1)*4),store_Traj{marker,4}(j:j+tl,plotdofs(2)+(nTraj-1)*4),store_Traj{marker,4}(j:j+tl,plotdofs(3)+(nTraj-1)*4),'-','LineWidth',3,'Color',[0 0 0 0.5]);
            hold on 
             if j+tl <=40
            rt = plot3((store_Traj{marker,2}(j:j+tl,plotdofs(1)+(nTraj-1)*4)),(store_Traj{marker,2}(j:j+tl,plotdofs(2)+(nTraj-1)*4)),(store_Traj{marker,2}(j:j+tl,plotdofs(3)+(nTraj-1)*4)),'-','LineWidth',3,'Color',[0 1 0 0.5]);
            hold on 
            ...
            elseif j<=40
                end1=40;
                rt = plot3((store_Traj{marker,2}(j:end1,plotdofs(1)+(nTraj-1)*4)),(store_Traj{marker,2}(j:end1,plotdofs(2)+(nTraj-1)*4)),(store_Traj{marker,2}(j:end1,plotdofs(3)+(nTraj-1)*4)),'-','LineWidth',3,'Color',[0 1 0 0.5]);
                hold on
            end   
            
            plot3(store_Traj{marker,4}(j+tl,plotdofs(1)+(nTraj-1)*4),store_Traj{marker,4}(j+tl,plotdofs(2)+(nTraj-1)*4),store_Traj{marker,4}(j+tl,plotdofs(3)+(nTraj-1)*4),'.','MarkerSize',30,'Color','black');
            hold on
...
l.Position = [0.0706 0.7005 0.4769 0.2188];
...
%            daspect([1,1,1])
 axis([-1,1,-2,2,-0.06,0])
% view(-8,18)
% axis([-0.6,0.6,-1.8,1.8,-0.04,-0.016])
% view(-39,43)
set(gcf,'color','white')
    figssm = gcf;
...
myWriter = VideoWriter('SSM_Rail_solutions_Adiabatic_med','MPEG-4');
