...
to1=find(alphaTOG>=0.1);
to2=find(alphaTOG>=0.15);
to3=find(alphaTOG>=0.2);

to = [1,40,40*2,40*3,40*4,40*5,40*6,40*7,40*8];
to2 = 1667+to;
to3 = 2500+to;
...
alphaT = alphaTOG(to(indk):to(indk+1));
ctspan = alphaT/(epsilon);

ctspanR = ctspan(1:end);
...
alphaT = alphaTOG(to2(indk-8):to2(indk-8+1));
ctspan = alphaT/(epsilon);
...
alphaT = alphaTOG(to3(indk-2*8):to3(indk-2*8+1));
ctspan = alphaT/(epsilon);
