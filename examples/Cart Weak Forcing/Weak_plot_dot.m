...
% zo = [1.3*exp(-1i*pi/2),1.3*exp(1i*pi/2),0.8*exp(-1i*pi/2),1.5*exp(1i*pi/2),1*exp(1i*pi/2),1*exp(-1i*pi/2)];
...
% ax1 = gca;
% for ind = 1:6
%   plot(ax1,tSP,F_I_net(ind,:),'-','LineWidth',3,'color',[0 0 0 0.5])
% hold on 
%   
% end    
% hold on 
% plot(ax1,tSP,F_an,'-','LineWidth',3,'color',[1 0 0 0.3])
...
% print(fig, 'cart_xcdot_weak.pdf' , '-dpdf' , '-r300' )