...
Dis_F = cell(1,jumps+1);
 pos_ts = find(tspan>0);
t_rand=randi([1,max(size(pos_ts))],1,100);
save('t_rand.mat','t_rand')
t_rand = sort(t_rand);
t_select_rand = t_rand(1:jumps:100);
T_dis = tspan(pos_ts(t_select_rand));

loren = @(t,y) lorenz_3D(t,y);
IC = [0.8;0.3;0.2];

[tk,yy] = ode45(loren,tspan(1:pos_ts(t_select_rand(1))),IC);
sigma = 10;
rho = 28;
beta = 8/3;
...
% Max_F = [Max_F,max(abs(F_a_new))];

Force_Lorenz = griddedInterpolant(tk(1:end),F_a_new(:),'spline');
Dis_F{1,1} = @(xa) Force_Lorenz(xa).*((xa >=tspan(1)) & (xa <=tspan(pos_ts(t_select_rand(1)))));



for ind = 2:max(size(T_dis))
 IC = [rand(1);rand(1);rand(1)];
 [tk,yy] = ode45(loren,tspan(pos_ts(t_select_rand(ind-1)):pos_ts(t_select_rand(ind))),IC);
 F_a_new = yy(:,1)/max(abs(yy(:,1)));

 Force_Lorenz = griddedInterpolant(tk(2:end),F_a_new(2:end),'spline');

 Dis_F{1,ind} = @(xa) Force_Lorenz(xa).*((xa >tspan(pos_ts(t_select_rand(ind-1)))) & (xa <=tspan(pos_ts(t_select_rand(ind)))));
 
 
end    

IC = [rand(1);rand(1);rand(1)];
 [tk,yy] = ode45(loren,tspan(pos_ts(t_select_rand(ind)):end),IC);
 
 F_a_new = yy(:,1)/max(abs(yy(:,1)));

 Force_Lorenz = griddedInterpolant(tk(2:end),F_a_new(2:end),'spline');

 Dis_F{1,ind+1} = @(xa) Force_Lorenz(xa).*((xa >tspan(pos_ts(t_select_rand(ind)))) & (xa <=tspan(end)));
 

Force_Dis_Lorenz = @(xa) (Dis_F{1,1}(xa)+Dis_F{1,2}(xa)+Dis_F{1,3}(xa)+Dis_F{1,4}(xa)+Dis_F{1,5}(xa)+Dis_F{1,6}(xa)+Dis_F{1,7}(xa)+Dis_F{1,8}(xa)+Dis_F{1,9}(xa)+Dis_F{1,10}(xa)+Dis_F{1,11}(xa));
F_a = Force_Dis_Lorenz(tspan);

save('Force_Dis.mat','Dis_F','Force_Dis_Lorenz')
% load('Force_Dis.mat','Dis_F','Force_Dis_Lorenz')

% tk = linspace(0,300,5000);
% F_a_new = Force_Dis_Lorenz(tk);
% figure
...
fig = figure 
F_a = Force_Dis_Lorenz(tspan);
test_t = pos_ts(1):pos_ts(t_select_rand(1));
plot(tspan(test_t),F_a(test_t),'-','LineWidth',3)
hold on 
for ind = 2:10
    test_t = pos_ts(t_select_rand(ind-1))+1:pos_ts(t_select_rand(ind));
    plot(tspan(test_t),F_a(test_t),'-','LineWidth',3)
    hold on 
end    
test_t = pos_ts(t_select_rand(ind))+1:pos_ts(end);
plot(tspan(test_t),F_a(test_t),'-','LineWidth',3)
xlabel('$t \,[$s$]$','Interpreter','latex');
ylabel('$g(t) \,[$N$]$','Interpreter','latex');
title("Discontinous Forcing")
axis([0 500 -1 3])
ax2 = axes( 'Position' ,[.18 .7 .2 .2]);
box on
grid on ;
test_t = pos_ts(1):pos_ts(t_select_rand(1));
plot(tspan(test_t),F_a(test_t),'-','LineWidth',3)
...
%%
xs=@(t,ta) real(V*([exp(D(1)*(t-ta));conj(exp(D(1)*(t-ta)));exp(D(3)*(t-ta));conj(exp(D(3)*(t-ta)));exp(D(5)*(t-ta));conj(exp(D(5)*(t-ta)))].*(VI*[0*ones(1,max(size(ta)));0*ones(1,max(size(ta)));0*ones(1,max(size(ta)));0*ones(1,max(size(ta)));0*ones(1,max(size(ta)));Force_Dis_Lorenz(ta)])));
...
save('Solution_1d.mat','xi_full_eval')
...
save('Solution_3d.mat','xi_full_eval3')

xi_sol0=xi_sol{1,1};
xi_sol3 = griddedInterpolant(tcalc(:),(xi_sol0(1,:)).','spline');
save('Xi_Sol3d.mat','xi_sol3')
...
save('Solution_5d.mat','xi_full_eval5')
...
load('Xi_Sol1d.mat','xi_sol1')
load('Solution_3d.mat','xi_full_eval3')
load('Solution_1d.mat','xi_full_eval')
load('Solution_5d.mat','xi_full_eval5')
load('Xi_Sol3d.mat','xi_sol3')
l1 = D(1);
l1c = conj(D(1));
l2 = D(3);
l2c = conj(D(3));
...
save('Coeff_SSM_Exact_O1d.mat','H_Coeff1','F_Coeff1');

 load('Coeff_SSM_Exact_O1d.mat','H_Coeff1','F_Coeff1');
...
save('Coeff_SSM_Exact_O2d.mat','H_Coeff2','F_Coeff2');

load('Coeff_SSM_Exact_O2d.mat','H_Coeff2','F_Coeff2');
...
% end
% fig = figure 
% plot(Epsilon,NMTET,'-o','MarkerSize',10,'LineWidth',3,'color','blue')
% xlabel('$\epsilon$','Interpreter','latex');
% ylabel('NMTE','Interpreter','latex');
% title('Normalised Mean Trajectory Error for varying $\epsilon$','Interpreter','latex')
% NMTET = [NMTET,NMTE];
% end
