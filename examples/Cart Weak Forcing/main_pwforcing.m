...
positive_L = 1;
tcalc = 0:0.01:tspan(end);%tspan(positive_L:end);
              
h = tcalc(2)-tcalc(1);
tnew = 0:h:tcalc(end);
FtG  = [sparse(2*n-1,max(size(tnew)));Force_Lorenz(tnew .').'];
phi = VI*FtG;% (rearrange IMP)
Ftn = phi(:,1:end-1);
Ftn_p1 = phi(:,2:end);
ExpoA = (exp(lambda*h)-1)./(lambda);
ExpoB = (exp(lambda*h))./(lambda.^2) - 1./(lambda.^2) - h./(lambda);
PHI = ExpoA.*Ftn + ExpoB.*(Ftn_p1-Ftn)/h;
et = 0:h:(tcalc(end)-tcalc(1));
Explambda = exp(lambda.*et(1:end-1));
eta = zeros(2*size(M,1),max(size(tnew)));
for j = 1:2*size(M,1)  
                    eta_d = conv(PHI(j,:),Explambda(j,:));
...
xi_solg = z.';
xi_full = griddedInterpolant(tcalc(:),xi_solg,'linear');
xi_full_eval = @(xa) xi_full(xa);
xi_sol1 = griddedInterpolant(tcalc(:),(z(1,:)).','linear');
...
FtG  = [0*ones(1,max(size(tnew)));0*ones(1,max(size(tnew)));0*ones(1,max(size(tnew)));-(m1+Mf)/(m1*Mf)*gamma*xi_sol1(tnew).^3;1/m1*gamma*xi_sol1(tnew).^3;0*ones(1,max(size(tnew)))];
% FtG  = [sparse(2*n-1,max(size(tnew)));Force_Lorenz(tnew .').'];
phi = VI*FtG;% (rearrange IMP)
Ftn = phi(:,1:end-1);
Ftn_p1 = phi(:,2:end);
...
xi_full = griddedInterpolant(tcalc(:),xi_solg,'linear');
xi_full_eval3 = @(xa) xi_full(xa);
xi_sol3 = griddedInterpolant(tcalc(:),(z(1,:)).','linear');
save('Solution_3.mat','xi_full_eval3')
save('Xi_Sol3.mat','xi_sol3')
...
FtG  = [0*ones(1,max(size(tnew)));0*ones(1,max(size(tnew)));0*ones(1,max(size(tnew)));3*(-(m1+Mf)/(m1*Mf))*gamma*xi_sol1(tnew).^2.*xi_sol3(tnew);3*1/m1*gamma*xi_sol1(tnew).^2.*xi_sol3(tnew);0*ones(1,max(size(tnew)))];
% FtG  = [sparse(2*n-1,max(size(tnew)));Force_Lorenz(tnew .').'];
phi = VI*FtG;% (rearrange IMP)
Ftn = phi(:,1:end-1);
Ftn_p1 = phi(:,2:end);
...
xi_full = griddedInterpolant(tcalc(:),xi_solg,'linear');
xi_full_eval5 = @(xa) xi_full(xa);
xi_sol5 = griddedInterpolant(tcalc(:),(z(1,:)).','linear');
save('Solution_5.mat','xi_full_eval5')
...
phih = h_coeff(0,tnew);% (rearrange IMP)
Ftn = phih(:,1:end-1);
Ftn_p1 = phih(:,2:end);
ExpoA = (exp(lambdah*h)-1)./(lambdah);
ExpoB = (exp(lambdah*h))./(lambdah.^2) - 1./(lambdah.^2) - h./(lambdah);
PHI = ExpoA.*Ftn + ExpoB.*(Ftn_p1-Ftn)/h;
et = 0:h:(tcalc(end)-tcalc(1));
Explambda = exp(lambdah.*et(1:end-1));
eta = zeros(size(phih,1),max(size(tnew)));
for j = 1:size(phih,1)  
                    eta_d = conv(PHI(j,:),Explambda(j,:));
                    ETA_M = eta_d(1:max(size(et(1:end-1))));
                    eta(j,:) = [0,ETA_M];
end
H_Coeff1=griddedInterpolant(tcalc(:),eta.','linear');

lambdaf = [l3-2.*l1c;l3-2.*l1;l3-l1c-l1;l3-l1c;l3-l1];
phih = f_coeff(0,tnew);% (rearrange IMP)
Ftn = phih(:,1:end-1);
Ftn_p1 = phih(:,2:end);
ExpoA = (exp(lambdaf*h)-1)./(lambdaf);
ExpoB = (exp(lambdaf*h))./(lambdaf.^2) - 1./(lambdaf.^2) - h./(lambdaf);
PHI = ExpoA.*Ftn + ExpoB.*(Ftn_p1-Ftn)/h;
et = 0:h:(tcalc(end)-tcalc(1));
Explambda = exp(lambdaf.*et(1:end-1));
eta = zeros(size(phih,1),max(size(tnew)));
for j = 1:size(phih,1)  
                    eta_d = conv(PHI(j,:),Explambda(j,:));
                    ETA_M = eta_d(1:max(size(et(1:end-1))));
...
F_Coeff1=griddedInterpolant(tcalc(:),eta.','linear');
...
lambdah = [l2-4.*l1;l2-4.*l1c;l2-l1c-3.*l1;l2-3.*l1c-l1;l2-2.*l1c-2.*l1;l2-3.*l1;l2-3.*l1c;l2-l1c-2.*l1;l2-2.*l1c-l1;l2-2.*l1;l2-2.*l1c;l2-l1c-l1;l2-l1;l2-l1c]; 
phih = h_coeff_2(0,tnew);% (rearrange IMP)
Ftn = phih(:,1:end-1);
Ftn_p1 = phih(:,2:end);
ExpoA = (exp(lambdah*h)-1)./(lambdah);
ExpoB = (exp(lambdah*h))./(lambdah.^2) - 1./(lambdah.^2) - h./(lambdah);
...
H_Coeff2=griddedInterpolant(tcalc(:),eta.','linear');
...
phih = f_coeff_2(0,tnew);% (rearrange IMP)
Ftn = phih(:,1:end-1);
Ftn_p1 = phih(:,2:end);
ExpoA = (exp(lambdaf*h)-1)./(lambdaf);
ExpoB = (exp(lambdaf*h))./(lambdaf.^2) - 1./(lambdaf.^2) - h./(lambdaf);
...
F_Coeff2=griddedInterpolant(tcalc(:),eta.','linear');
...
SolutionNA = epsilon*xi_full_eval(ctspan.') +epsilon^3*xi_full_eval3(ctspan.')+epsilon^5*xi_full_eval5(ctspan.');
...
hold on 
plot(ctspan,y_linear(indexR,:),'-','LineWidth',3,'color',[0 1 0 0.3])
hold on
% plot(ctspan,SolutionA(:,indexR),'-','LineWidth',3,'color',[0 1 0 0.3])
hold on 
plot(ctspan,SolutionNA(:,indexR),'-','LineWidth',3,'color',[0 0 1 0.3])
