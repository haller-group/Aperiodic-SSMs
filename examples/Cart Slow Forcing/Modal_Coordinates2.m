...
SP_Traj1 = SP_Traj.';
nssm = 20;
for j = 1:1%2550 %1:20:5000%(max(size(t_sol))-tl)
i = j+0;
...
%      hold on
     ...
     plot3(SolutionZ(plotdofs(1),:),SolutionZ(plotdofs(2),:),SolutionZ(plotdofs(3),:),'.','LineWidth',3,'color',[0,0,1,0.5])
     hold on
     ...
     hold on
     plot3(SolutionZ(plotdofs(1),j:i+tl),SolutionZ(plotdofs(2),j:i+tl),SolutionZ(plotdofs(3),j:i+tl),'-','LineWidth',3,'color',[0 1 1 0.5])
     hold on
...
%      hold on
     ...
     plot3(SP_TrajZ(plotdofs(1),i+tl),SP_TrajZ(plotdofs(2),i+tl),SP_TrajZ(plotdofs(3),i+tl),'.','MarkerSize',30,'Color','black');
     hold on
     plot3(SolutionZ(plotdofs(1),i+tl),SolutionZ(plotdofs(2),i+tl),SolutionZ(plotdofs(3),i+tl),'.','MarkerSize',30,'Color','cyan');
     hold on
        
    xlabel('Re[$u_1+\xi_{u_1}$] [m]','Interpreter','latex');
     ylabel('Im[$u_1+\xi_{u_1}$] [m]','Interpreter','latex');
     zlabel('Re[$v_2+\xi_{v_2}$] [m]','Interpreter','latex');
...
%     string = strcat('t = ',num2str(round(t_sol(j+tl),2),'%4.2f' ),', \epsilon = 0.5');
%     if (j+tl) >=2000
...
% myWriter = VideoWriter('Shaking_Cart_SSM_video_medium','MPEG-4');
...
set(hFig, 'PaperPositionMode' , 'Auto' , 'PaperUnits' , 'Inches' , 'PaperSize' ,[pos(3), pos(4)])
set(gcf,'Renderer','painters')
print(hFig, 'ssm_shaking_cart.pdf' , '-dpdf' , '-r300' )